---
name: qwen-work-assigner
description: Delegate menial, well-scoped coding tasks to a cheap Qwen-backed subagent via the `claude-qwen` command instead of burning Claude tokens/quota. Use when the work is mechanical and low-risk — bulk renames, formatting, boilerplate, find-replace, grep-style search & summarization, reading/condensing logs or files, test/docstring/comment scaffolding, or running builds/linters/tests and reporting pass-fail. Also use when the user says "use qwen", "delegate this", "send it to larviets/qwen", or "do this cheaply". Do NOT use for architecture, design, debugging judgment, security-sensitive edits, or anything needing this conversation's context.
---

# qwen-work-assigner

Offload **menial, self-contained** tasks to a Qwen model running inside a headless Claude Code instance (`claude-qwen`). Keeps expensive Claude reasoning for work that needs it.

## The command

`claude-qwen` is a shell alias → `claude --model qwen3.6-35b-a3b` routed through the qwen larviets local server. Run it headless with `-p`:

```bash
claude-qwen -p "load `qwen-chance` skill and do the prompt on the right / <self-contained task prompt>" --allowedTools Bash Read Edit Write Glob Grep
```
...
- **Break large jobs into independent chunks** that each touch a bounded slice — e.g. one file (or a few small ones) per run, one directory per run, one log segment per run. Run the chunks as separate `claude-qwen` invocations (foreground, or background-parallel per the Return contract section).
...
- **Background / parallel (run several at once):** redirect to a log and run with the Bash tool's `run_in_background: true`, then read the log when it finishes:
  ...
  ```bash
  claude-qwen -p "<task>" --allowedTools Bash Read Edit Write Glob Grep > /tmp/qwen-<label>.log 2>&1
  ```

  IMPORTANT: Launch independent tasks as separate background runs; collect each log on completion. Use this when delegating 2+ unrelated menial jobs.
...
4. Run `claude-qwen -p "..." --allowedTools Bash Read Edit Write Glob Grep` (foreground), or background-redirect for parallel jobs.
...
```json
{ "permissions": { "allow": ["Bash(claude-qwen:*)"] } }
```
